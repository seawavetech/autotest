    ...
    <el-row style="background-color: #252d35;">
        <el-col>
            <el-switch
                    ...
                    class="ml-2"
                    inline-prompt
                    style="--el-switch-on-color: #45d7c4; --el-switch-off-color: #868686"
                    active-text="展开"
                    inactive-text="收起" />
                    ...
                    <el-sub-menu :index="`${index}`" v-if="nav.children && nav.children.length > 0">
                        ...
                        <template v-for="(nav2,index2) in nav.children" :key="index2">
                            <el-menu-item-group :title="nav2.title" v-if="nav2.group && nav2.group.length > 0">
                                <el-menu-item :index="`${index}-${index2}-${index3}`" v-for="(nav3,index3) in nav2.group" :key="index3">
        ...
        children:[
            {
                ...
                group:[
                    ...
                    {
                        title: 'M端测试',
                        url:'/test?site=dad&platform=m&range=all'
                    }
                ]
            },
            {
                title:'Dorris Wedding',
                url:'',
                group:[
                    {
                        title: 'PC端测试',
